...
try{
    ...
    for(const [c,i] of selected){if(c[5]===8){const actual=compare(i,()=>{reset(i,{offset:0xFFF,cpl:3});cpu.reg32[4]=0x12348000;set32(0x13000+0x311*4,0);e.full_clear_tlb();},101);assert.equal(actual.ip,PF);assert.equal(actual.regs[4],STACK-24);sourceFaults++;}
        const actual=compare(i,()=>{reset(i,{value:0x1000,cpl:3});cpu.reg32[4]=0x12348000;mem.copyWithin(0x5000,0x3000,0x3040);cpu.gdtr_offset[0]=0x5000;cpu.gdtr_size[0]=0x10FF;set32(0x13000+5*4,0x5003);set32(0x13000+6*4,0);e.full_clear_tlb();},101);assert.equal(actual.ip,PF);assert.equal(actual.cr[2],0x6000);if(c[7]===4)assert.equal(actual.regs[4],c[2]===16?0x88000:0x12348000);else assert.equal(actual.regs[4],STACK-24);descriptorFaults++;
    }
    ...
    console.log(`PASS (${release?"release":"debug"}): ${sourceFaults} source #PF, ${descriptorFaults} descriptor #PF/post-fault destination writes, ${segments} segment-priority cases`);
    ...
    let devices=0,lateOriginal=0,local=0;
    for(const [c,i] of selected){compare(i,()=>{reset(i);if(c[5]===8)set32(0x13000+0x310*4,0xA0003);set32(0x13000+3*4,0xA1003);e.full_clear_tlb();});assert(events.length>0);devices++;
        if(c[5]===8){const actual=compare(i,()=>{reset(i,{value:0});set32(0x13000+0x310*4,0xA0003);e.full_clear_tlb();onEvent=()=>{cpu.reg32[c[7]]=0x1234ABCD;};});assert.equal(actual.regs[c[7]],0x1234ABCD);lateOriginal++;}
        const actual=compare(i,()=>{reset(i,{value:0x34});mem.copyWithin(0x70000,0x3000,0x3040);});assert(actual.flags&64);local++;
    }
    console.log(`PASS (${release?"release":"debug"}): ${devices} operand/descriptor MMIO, ${lateOriginal} post-source original-value captures, ${local} LDT lookups`);
