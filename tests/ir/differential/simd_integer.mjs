...
for(const release of [false,true]){
 ...
 try {
    ...
    const chain=extra("chain"),resume=extra("resume"),chain_bytes=JSON.parse(fs.readFileSync("build/ir-simd-integer/chain.json"));
    ...
    for(let i=0;i<cases.length;i++) for(const hot of cases[i][7]<8?[false]:[false,true]){
        ...
        const {counts}=compare(i,()=>reset(i,{hot}),{check:(before,data,after)=>{
            ...
            expected.splice(reg*4,4,...packed(c[3],before.xmm.slice(reg*4,reg*4+4),operand<8?before.xmm.slice(operand*4,operand*4+4):data));
    ...
    console.log(`PASS (${release?"release":"debug"}): ${ordinary} independent/CPU packed SIMD scenarios, ${native} native RAM entries`);
    let randomized=0;
    ...
    console.log(`PASS (${release?"release":"debug"}): ${randomized} independent packed boundary/random/alias cases`);
    let counts_checked=0,narrows=0;
    const verify_packed=(c,before,data,after)=>{
        const expected=before.xmm.slice(),reg=c[6],operand=c[7];
        expected.splice(reg*4,4,...packed(c[3],before.xmm.slice(reg*4,reg*4+4),operand<8?before.xmm.slice(operand*4,operand*4+4):data));
        assert.deepEqual(after.xmm,expected);
    };
    for(let i=0;i<cases.length;i++){
        const c=cases[i];if(!c[1]||c[2]!==32||c[6]!==0||![0,1,8].includes(c[7])) continue;
        if([0xD1,0xD2,0xD3,0xE1,0xE2,0xF1,0xF2,0xF3].includes(c[3]&255)){
            for(const count of [0n,1n,7n,8n,15n,16n,17n,31n,32n,33n,63n,64n,65n,127n,128n,255n,65535n,1n<<32n,(1n<<32n)+1n,1n<<63n,(1n<<64n)-1n])
            for(const high of [0,0xFFFFFFFF]) for(const hot of c[7]<8?[false]:[false,true]){
                compare(i,()=>{
                    reset(i,{hot});xmm.set([0x8001FFFF,0x7FFFFFFF,0xFFFFFFFF,0x80000000],0);
                    const value=[Number(count&0xFFFFFFFFn),Number(count>>32n),high,high];
                    if(c[7]<8)xmm.set(value,c[7]*4);else value.forEach((n,j)=>set32(target+j*4,n));
                },{check:(before,data,after)=>verify_packed(c,before,data,after)});counts_checked++;
            }
        }
        if([0x63,0x67,0x6B].includes(c[3]&255)){
            const edges=[-32769,-32768,-32767,-257,-256,-129,-128,-127,-1,0,1,126,127,128,129,254,255,256,257,32766,32767,32768,32769];
            for(let seed=0;seed<edges.length;seed++) for(const hot of c[7]<8?[false]:[false,true]){
                compare(i,()=>{
                    reset(i,{hot});const bits=(c[3]&255)===0x6B?32:16;
                    const fill=(n,j)=>bits===32?n>>>0:(n&65535)|(edges[(seed+j+1)%edges.length]&65535)<<16;
                    for(let j=0;j<32;j++)xmm[j]=fill(edges[(seed+j)%edges.length],j);
                    for(let j=0;j<4;j++)set32(target+4*j,fill(edges[(seed+j+7)%edges.length],j+7));
                },{check:(before,data,after)=>verify_packed(c,before,data,after)});narrows++;
            }
        }
    }
    console.log(`PASS (${release?"release":"debug"}): ${counts_checked} full-u64 count/high-half cases and ${narrows} signed narrowing boundaries`);
    ...
    for(const [c,i] of selected){
        ...
        for(const hot of [false,true]){
            const {expected,counts}=compare(i,()=>{
                ...
                // A zero low-qword count never licenses skipping the upper read.
                if([0xD1,0xD2,0xD3,0xE1,0xE2,0xF1,0xF2,0xF3].includes(c[3]&255)){set32(target,0);set32(target+4,0);}
    ...
    console.log(`PASS (${release?"release":"debug"}): ${widths} exact 8/16-byte read extents with absent upper page`);
    ...
    for(const [c,i] of selected){
        ...
        compare(i,()=>{device(i);on_event=()=>{xmm[0]=0x12345678;xmm[28]=0x12345678;cpu.reg32[0]=0xCAFEBABE;};});assert.equal(xmm[28],0x12345678);assert.equal(cpu.reg32[0]>>>0,0xCAFEBABE);mutations++;
        if(c[5]){
            const {expected}=compare(i,()=>{device(i,{offset:0xFFC,cpl:3});on_event=kind=>{if(kind.startsWith("w")){set32(0x13000+0x311*4,0);e.full_clear_tlb();}};},{abort:c[4]>4});
            if(c[4]>4){assert.equal(expected.ip,PF);late++;}
        } else {
            const {expected}=compare(i,()=>{device(i,{offset:0xFFC,cpl:3});on_event=kind=>{if(kind.startsWith("r")){xmm[0]=0xFEDCBA98;set32(0x13000+0x311*4,0);e.full_clear_tlb();}};},{fault:c[4]>4});if(c[4]>4){assert.equal(expected.ip,PF);late++;}
        }
    ...
    for(const [c,i] of selected){
        ...
        for(const offset of [0x40,0xFFC]){
            const {observed}=compare(i,()=>{device(i,{offset});on_event=kind=>{
    ...
    const base_case=cases.findIndex(c=>c[1]&&c[2]===32&&c[3]===0x660FFC&&c[6]===0&&c[7]===8);
    const setup_chain=(hot,deviceLoad=false)=>{reset(base_case);cpu.segment_offsets.fill(0,0,6);cpu.reg32[6]=BASE+0x40;cpu.reg32[7]=BASE+0x10040;mem.fill(0xCC,BASE+0x10040,BASE+0x10050);mem.set(chain_bytes,PC);e.full_clear_tlb();if(hot){e.ir_memory_write(BASE+0x40,mem[BASE+0x40],1);e.ir_memory_write(BASE+0x10040,mem[BASE+0x10040],1);} if(deviceLoad){set32(0x13000+0x310*4,0xA0007);e.full_clear_tlb();on_event=kind=>{if(kind.startsWith("r")){xmm[4]=0xDEADBEEF;xmm[24]=0xDEADBEEF;}};}slow=guards=0;events=[];};
    let chains=0;
    for(const hot of [false,true]) for(const device_load of [false,true]){
        const steps=hot&&!device_load?6:3;setup_chain(hot,device_load);for(let n=0;n<steps;n++)e.ir_test_step();const expected=state(),observed=events.slice();for(let n=steps;n<6;n++)e.ir_test_step();const completed=state(),out=Buffer.from(mem.slice(BASE+0x10040,BASE+0x10050));
        for(const opt of [0,1]){setup_chain(hot,device_load);chain[opt].exports.f(0);assert.equal(words[664>>2],100+steps);assert.deepEqual(state(),expected);assert.deepEqual(events,observed);if(steps===3)resume[opt].exports.f(0);assert.equal(words[664>>2],106);assert.deepEqual(state(),completed);assert.deepEqual(Buffer.from(mem.slice(BASE+0x10040,BASE+0x10050)),out);chains++;}
    }
    ...
    console.log(`PASS (${release?"release":"debug"}): ${chains} packed SSA/slow completion/resume chains and 2 dirty-XMM fault exits`);
