...
const files = [
    ...
    "v86_network_bridge.js", "graphics_adapter.js",
];
const contents = await Promise.all(files.map(file => readFile(new URL(file, source), "utf8")));
const worker = await readFile(new URL("d3d9-webgpu/d3d9_shader_worker.js", source), "utf8");
const revision = createHash("sha256").update(contents.join("\n") + worker).digest("hex").slice(0, 20);
...
await writeFile(new URL("libv86-webgpu.js", output), bundle);
await writeFile(new URL("d3d9_shader_worker.js", output), prefix + worker);
await writeFile(new URL("d3d9_shader_pipeline.js", output), prefix + contents[2]);
await writeFile(new URL("manifest.json", output), JSON.stringify({ revision, files: [
    "libv86-webgpu.js", "d3d9_shader_worker.js", "d3d9_shader_pipeline.js",
] }, null, 2) + "\n");
