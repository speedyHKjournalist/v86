...
#[test]
fn task_regs_fixtures() {
    std::fs::create_dir_all("build/ir-task-regs").unwrap();
                    ...
                    for operand in 0..15 {
                        for rep in [0, 0xF2, 0xF3] {
                            ...
                            for opt in 0..2 {
                                ...
                                std::fs::write(
                                    format!("build/ir-task-regs/{}-{opt}.wasm", cases.len()),
    ...
    std::fs::write(
        "build/ir-task-regs/cases.json",
        format!("[{}]", cases.join(",")),
    )
    .unwrap();
}
#[test]
fn task_regs_terminal_contract() {
    for group in [0, 1, 2, 3] {
        for reg in [false, true] {
            let bytes = [0x0F, 0x00, group << 3 | if reg { 0xC0 } else { 6 }];
