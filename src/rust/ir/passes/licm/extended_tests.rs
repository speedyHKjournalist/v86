...
fn budget_failures_are_atomic() {
    ...
    assert!(run(&mut invalid, Config::default()).is_err());
...
fn emits_optimized_and_reference_modules() {
    ...
    std::fs::create_dir_all("build/ir-licm-extended").unwrap();
    for budget in [1, 2, 3, 4, 5, 9, 16, 100] {
        for optimized in [false, true] {
            ...
            std::fs::write(
                format!("build/ir-licm-extended/loop-{budget}-{optimized}.wasm"),
