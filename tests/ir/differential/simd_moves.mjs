...
const cases=JSON.parse(fs.readFileSync("build/ir-simd-moves/cases.json"));
const modules=cases.map((_,i)=>[0,1].map(opt=>new WebAssembly.Module(fs.readFileSync(`build/ir-simd-moves/${i}-${opt}.wasm`))));
...
for(const release of [false,true]){
 ...
 try {
    ...
    const imports={...e,m:e.memory,ir_xmm_load:(...a)=>{slow++;return e.ir_xmm_load(...a);},ir_xmm_store:(...a)=>{slow++;return e.ir_xmm_store(...a);},ir_sse_guard:()=>{guards++;return e.ir_sse_guard();}};
    const instances=modules.map(pair=>pair.map(m=>new WebAssembly.Instance(m,{e:imports})));
    const extra=name=>[0,1].map(opt=>new WebAssembly.Instance(new WebAssembly.Module(fs.readFileSync(`build/ir-simd-moves/${name}-${opt}.wasm`)),{e:imports}));
    const chain=extra("chain"),resume=extra("resume"),edge=extra("edge"),chain_bytes=JSON.parse(fs.readFileSync("build/ir-simd-moves/chain.json"));
    ...
    function compare(i,configure,{fault=false,abort=false,check}={}){
        ...
        const counts=[];for(const opt of [0,1]){configure();assert.equal(caught(()=>instances[i][opt].exports.f(0)),abort);assert.equal(words[664>>2],fault||abort?101:102);assert.deepEqual(state(),expected,`SSE move ${i}/${opt}`);assert.deepEqual(events,observed,`events ${i}/${opt}`);counts.push([slow,guards]);} return {expected,observed,counts};
    }
    ...
    for(let i=0;i<cases.length;i++) for(const hot of cases[i][7]<8?[false]:[false,true]){
        ...
        const {counts}=compare(i,()=>reset(i,{hot}),{check:(before,data,after)=>{
            const expected=before.xmm.slice();if(operand<8){const dst=store?operand:reg,src=store?reg:operand;expected.splice(dst*4,width/4,...before.xmm.slice(src*4,src*4+width/4));}
            else if(!store){expected.fill(0,reg*4,reg*4+4);expected.splice(reg*4,width/4,...data.slice(0,width/4));}
            assert.deepEqual(after.xmm,expected);assert.deepEqual(after.regs,before.regs);assert.equal(after.flags,before.flags);
            if(operand>=8&&store) for(let l=0;l<4;l++)assert.equal(get32(target+l*4),l<width/4?before.xmm[reg*4+l]:data[l]);
        }});assert(counts.every(([n,g])=>g===0&&n===(c[7]<8||hot?0:1)));ordinary++;if(c[7]>=8&&hot)native+=2;
    }
    console.log(`PASS (${release?"release":"debug"}): ${ordinary} independent/CPU SIMD move scenarios, ${native} native RAM entries`);
    ...
    for(const [c,i] of selected){
        ...
        compare(i,()=>{device(i);on_event=()=>{xmm[28]=0x12345678;cpu.reg32[0]=0xCAFEBABE;};});assert.equal(xmm[28],0x12345678);assert.equal(cpu.reg32[0]>>>0,0xCAFEBABE);mutations++;
        if(c[5]){
    ...
    const base_case=cases.findIndex(c=>c[1]&&c[2]===32&&c[3]===0x0F10&&c[6]===0&&c[7]===8);
    const setup_chain=(hot,deviceLoad=false)=>{reset(base_case);cpu.segment_offsets.fill(0,0,6);cpu.reg32[6]=BASE+0x40;cpu.reg32[7]=BASE+0x10040;mem.fill(0xCC,BASE+0x10040,BASE+0x10050);mem.set(chain_bytes,PC);e.full_clear_tlb();if(hot){e.ir_memory_write(BASE+0x40,mem[BASE+0x40],1);e.ir_memory_write(BASE+0x10040,mem[BASE+0x10040],1);} if(deviceLoad){set32(0x13000+0x310*4,0xA0007);e.full_clear_tlb();on_event=kind=>{if(kind.startsWith("r"))xmm[24]=0xDEADBEEF;};}slow=guards=0;events=[];};
    let chains=0;
    for(const hot of [false,true]) for(const device_load of [false,true]){
        const steps=hot&&!device_load?5:3;setup_chain(hot,device_load);for(let n=0;n<steps;n++)e.ir_test_step();const expected=state(),observed=events.slice();for(let n=steps;n<5;n++)e.ir_test_step();const completed=state(),out=Buffer.from(mem.slice(BASE+0x10040,BASE+0x10050));
        for(const opt of [0,1]){setup_chain(hot,device_load);chain[opt].exports.f(0);assert.equal(words[664>>2],100+steps);assert.deepEqual(state(),expected);assert.deepEqual(events,observed);if(steps===3)resume[opt].exports.f(0);assert.equal(words[664>>2],105);assert.deepEqual(state(),completed);assert.deepEqual(Buffer.from(mem.slice(BASE+0x10040,BASE+0x10050)),out);chains++;}
    }
    ...
    for(const opt of [0,1]){reset(base_case);const expected=Array.from(xmm);expected.splice(0,8,...expected.slice(4,8),...expected.slice(0,4));edge[opt].exports.f(0);assert.deepEqual(Array.from(xmm),expected);assert.equal(words[664>>2],101);}
    console.log(`PASS (${release?"release":"debug"}): ${chains} V128 SSA chains, cold completion exits and resumed vector stores, 2 coalesced vector edge copies and 2 dirty-XMM fault exits`);
