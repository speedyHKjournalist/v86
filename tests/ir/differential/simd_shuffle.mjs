...
for(const release of [false,true]){
 ...
 try{
    ...
    const instances=boundedInstances(i=>[0,1].map(opt=>new WebAssembly.Instance(new WebAssembly.Module(fs.readFileSync(`build/ir-simd-shuffle/${i}-${opt}.wasm`)),{e:imports})));
