...
const repetitions = positiveInteger('IR_COMPARE_RUNS', 3, 3);
const updates = positiveInteger('IR_BOUNDARY_UPDATES', 8_000_000, 16);
const warmRuns = positiveInteger('IR_BOUNDARY_WARM_RUNS', 20, 20);
const warmIterations = positiveInteger('IR_BOUNDARY_WARM_ITERATIONS', 20_000, 20_000);
const timeout = positiveInteger('IR_BOUNDARY_TIMEOUT_MS', 60_000, 1000);
...
function makeWork(shape, body) {
    ...
    const updatesPerIteration = pages.length * body;
    return {
        name: `${shape}_${body * 3}`, shape, body, chunks, end,
        entries: shape === 'page_boundaries' ? pages : [PC],
        iterations: updates / updatesPerIteration,
        updatesPerIteration,
        instructionsPerIteration: updatesPerIteration * 3 + 2 + (shape === 'straight' ? 0 : pages.length - 1),
    };
}
const workloads = bodies.flatMap(body => shapes.map(shape => makeWork(shape, body)));
for(const work of workloads) {
    ...
    assert(warmIterations * work.instructionsPerIteration + 3 < 2 ** 32, 'warm retirement delta must not wrap');
}
...
for(const work of workloads) for(let round = 0; round < repetitions; round++) {
    ...
    for(const arm of ordered) {
        ...
        try {
            ...
            vm.run();
            const bootDeadline = performance.now() + 15_000;
            while(data().getUint16(0x500, true) !== 0xCAFE) {
                assert(performance.now() < bootDeadline, `${context}: BIOS timeout`);
                await sleep(1);
            }
            await vm.stop();
            for(const { address, bytes } of work.chunks) vm.write_memory(Uint8Array.from(bytes), address);
            const readStats = () => Object.fromEntries(Object.entries(statFields).map(([name, field]) =>
                [name, typeof e.ir_cache_stat === 'function' ? e.ir_cache_stat(field) >>> 0 : 0]));
            const entryTier = pc => e.ir_cache_entry_stat(pc, 0, 1, 5);
            ...
            const run = async iterations => {
                ...
                const beforeIr = readStats(), beforeSteps = vm.get_instruction_counter() >>> 0;
            ...
            let warmed = 0;
            do {
                await run(warmIterations);
                await sleep(1);
                warmed++;
            } while(warmed < warmRuns || (backend === 'ir' && warmed < warmRuns + 40 &&
                work.entries.some(pc => entryTier(pc) !== 2)));
            if(backend === 'ir') {
                for(const pc of work.entries) {
                    assert.equal(entryTier(pc), 2, `${context}: 0x${pc.toString(16)} must reach Tier 2`);
            ...
            const key = `${round}`;
            if(pairedStates.has(key)) assert.deepEqual(result.state, pairedStates.get(key), `${context}: architectural parity`);
            else pairedStates.set(key, result.state);
            const row = {
...
const matrix = variants.flatMap(({ label }) => bodies.map(body => {
    const byBackend = Object.fromEntries(['ir', 'legacy'].map(backend => [backend,
        Object.fromEntries(shapes.map(shape => [shape, timing(label, backend, body, shape)]))]));
    return {
        variant: label, arithmetic_per_fragment: body * 3, ...byBackend,
        ir_over_legacy: Object.fromEntries(shapes.map(shape => [shape,
            byBackend.legacy[shape].ns_per_update / byBackend.ir[shape].ns_per_update])),
