...
fn fixture() -> Fixture {
    ...
    let entry = b.block;
    let input = b.gpr;
    let read = instruction(&b.region, input[4]);
    ...
    b.region.terminate(
        entry,
        Terminator::Branch(Edge { target: header, args: vec![b.effect, input[2], zero] }),
    );
...
fn hoists_dependency_chain_and_preserves_recovery_effects_and_edges() {
    ...
    let blocks: Vec<_> = f
        ...
        .blocks
        .iter()
        .map(|b| format!("{:?} {:?} {:?}", b.params, b.terminator, b.entry_state))
        .collect();
    ...
    assert_eq!(f.region.instructions[f.read.index()].block, f.entry);
    for id in [f.dependent, f.polls[0], f.polls[1]] {
...
fn emit_licm_reference_and_optimized_modules() {
    ...
    for budget in [1, 2, 3, 4, 5, 8, 16, 64, 256] {
        for optimized in [false, true] {
            ...
            drop(f.region);
            std::fs::write(
                format!("build/ir-licm-safety/loop-{budget}-{optimized}.wasm"),
