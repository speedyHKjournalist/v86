...
| IR-10 | 部分基础 | 有界常量折叠、支配关系 GVN、trivial phi 消除、StateMap-aware DCE、常量分支裁剪和保留预算检查的直线块合并；已有独立 MIR 字面量常量折叠；新增纯 CPU CFG 的入口等价 backing-state 证明，可在 Tier 2 预算恢复/出口省略未变化 GPR/XMM/FLAGS 写回；可继续 helper/MMU 观察后的通用 dirty-state 合流及更细粒度 FLAGS liveness 仍未完成 |
...
- lowering 在 HIR/CFG edge 仍可见时，对 GPR、XMM、EFLAGS system、CF/PF/AF/ZF/SF/OF、last_op1、raw ZF 与 lazy-ZF 来源做固定点传播，生成每个 StateMap 的 CPU write 省略证书。
...
- EIP、previous_ip 与退休计数永远保留；发生算术 FLAGS 或寄存器变化时对应写回继续保留。Tier 1、standalone、关闭优化和 zero-round 配置不启用。
