...
impl WorkBudget {
    ...
    pub fn charge(&mut self, amount: usize) -> Result<(), String> {
        let next = self.used.checked_add(amount).ok_or("loop work overflow")?;
        if next > self.limit {
            return Err("loop work budget exceeded".into());
        }
        self.used = next;
        Ok(())
    }
...
impl LoopAnalysis {
    pub fn compute(region: &Region, work: &mut WorkBudget) -> Result<Self, String> {
        let n = region.blocks.len();
        if n > MAX_BLOCKS {
            return Err("loop block budget exceeded".into());
        }
