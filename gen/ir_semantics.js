const remaining = new Set([0x62,0x63,0x9B,0xF1,0xF04,0xF05,0xF07,0xF08,0xF0A,0xF0B,0xF0C,0xF0D,0xF0E,0xF0F,0xF18,0xF19,0xF1A,0xF1B,0xF1C,0xF1D,0xF1E,0xF1F,0xF24,0xF25,0xF26,0xF27,0xF33,0xF36,0xF37,0xF38,0xF39,0xF3A,0xF3B,0xF3C,0xF3D,0xF3E,0xF3F,0xF6C,0xF6D,0xF78,0xF79,0xF7A,0xF7B,0xF7C,0xF7D,0xFA6,0xFA7,0xFAA,0xFAE,0xFB8,0xFB9,0xFC3,0xFC7,0xFD0,0xFD6,0xFE6,0xFF0,0xFFF]);
const mmx = new Set([0xF60,0xF61,0xF62,0xF63,0xF64,0xF65,0xF66,0xF67,0xF68,0xF69,0xF6A,0xF6B,0xF6E,0xF6F,0xF70,0xF74,0xF75,0xF76,0xFC4,0xFD1,0xFD2,0xFD3,0xFD4,0xFD5,0xFD8,0xFD9,0xFDA,0xFDB,0xFDC,0xFDD,0xFDE,0xFDF,0xFE0,0xFE1,0xFE2,0xFE3,0xFE4,0xFE5,0xFE8,0xFE9,0xFEA,0xFEB,0xFEC,0xFED,0xFEE,0xFEF,0xFF1,0xFF2,0xFF3,0xFF4,0xFF5,0xFF6,0xFF8,0xFF9,0xFFA,0xFFB,0xFFC,0xFFD,0xFFE,0xF77,0xF7E,0xF7F,0xFC5,0xFD7,0xFE7,0xF20FD6,0xF30FD6,0xFF7]);
const sse_fp = new Set([0x0F2A,0x0F2C,0x0F2D,0x0F2E,0x0F2F,0x0F51,0x0F52,0x0F53,0x0F58,0x0F59,0x0F5A,0x0F5B,0x0F5C,0x0F5D,0x0F5E,0x0F5F,0x0FC2,0x660F2A,0x660F2C,0x660F2D,0x660F2E,0x660F2F,0x660F51,0x660F58,0x660F59,0x660F5A,0x660F5B,0x660F5C,0x660F5D,0x660F5E,0x660F5F,0x660F7C,0x660F7D,0x660FC2,0x660FD0,0x660FE6,0xF20F2A,0xF20F2C,0xF20F2D,0xF20F51,0xF20F58,0xF20F59,0xF20F5A,0xF20F5C,0xF20F5D,0xF20F5E,0xF20F5F,0xF20F7C,0xF20F7D,0xF20FC2,0xF20FD0,0xF20FE6,0xF30F2A,0xF30F2C,0xF30F2D,0xF30F51,0xF30F52,0xF30F53,0xF30F58,0xF30F59,0xF30F5A,0xF30F5B,0xF30F5C,0xF30F5D,0xF30F5E,0xF30F5F,0xF30FC2,0xF30FE6]);
const xmm_lane = new Set([0x0F50,0x660F50,0x660FD7,0x660FC4,0x660FC5,0x0F2B,0x660F2B,0x660FE7,0xF20FF0]);
...
function experimental_lowering(encoding, operand) {
    ...
    if(xmm_lane.has(op)) return "CpuSimdHIR";
    if(xmm_transfer.has(op)) return "CpuSimdHIR";
    if(xmm_shuffle.has(op)) return "CpuSimdHIR";
    if(xmm_moves.has(op) || xmm_integer.has(op)) return "CpuSimdHIR";
    ...
    if([0xC4,0xC5,0x0FB2,0x0FB4,0x0FB5].includes(op)) return "CpuStateHIR";
    ...
    if(operand === "mem") {
        ...
        if(op >= 0x88 && op <= 0x8B || op === 0xC6 || op === 0xC7 ||
            ...
            [0xFE, 0xFF].includes(op) && g < 2 || [0xF6, 0xF7].includes(op) && [0,1,2,3].includes(g) ||
    ...
    if(op >= 0xB0 && op <= 0xBF || op >= 0x88 && op <= 0x8B || op === 0xC6 || op === 0xC7 ||
        ...
        (op === 0xF6 || op === 0xF7) && [0, 1, 2, 3].includes(g) ||
...
export { experimental_lowering as experimentalLowering };

// Suite attribution is evidence navigation, not exhaustive per-form acceptance.
function experimental_tests(encoding, operand) {
    const op = encoding.opcode, g = encoding.fixed_g;
    const category = experimental_lowering(encoding, operand);
    if(category === "Pending") return [];
    let suite;
    if(category === "CpuX87Helper") suite = operand === "reg" ? "x87" : "x87_memory";
    else if(category === "CpuFarControlHelper") suite = "far_control";
    else if(category === "CpuStiHIR") suite = "sti";
    else if(category === "CpuBaselineHelper") suite = "coverage";
    else if(category === "CpuMmxHelper") suite = "mmx";
...
export { experimental_tests as experimentalTests };
