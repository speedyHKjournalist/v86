...
for(const release of [false,true]){
 ...
 try {
    ...
    const chain=extra("chain"),resume=extra("resume"),bridge=extra("bridge"),chain_bytes=JSON.parse(fs.readFileSync("build/ir-simd-transfer/chain.json"));
    ...
    const base_case=cases.findIndex(c=>c[1]&&c[2]===32&&c[3]===0x0F16&&c[6]===0&&c[7]===8);
    const setup_chain=(hot,deviceLoad=false)=>{reset(base_case);cpu.segment_offsets.fill(0,0,6);cpu.reg32[6]=BASE+0x40;cpu.reg32[7]=BASE+0x10040;mem.fill(0xCC,BASE+0x10040,BASE+0x10050);mem.set(chain_bytes,PC);e.full_clear_tlb();if(hot){e.ir_memory_write(BASE+0x40,mem[BASE+0x40],1);e.ir_memory_write(BASE+0x10040,mem[BASE+0x10040],1);} if(deviceLoad){set32(0x13000+0x310*4,0xA0007);e.full_clear_tlb();on_event=kind=>{if(kind.startsWith("r")){xmm[8]=0xDEADBEEF;xmm[24]=0xDEADBEEF;}};}slow=guards=0;events=[];};
    let chains=0;
    ...
    const bridge_bytes=JSON.parse(fs.readFileSync("build/ir-simd-transfer/bridge.json"));
    const setup_bridge=()=>{reset(base_case);mem.set(bridge_bytes,PC);};
    setup_bridge();e.ir_test_step();e.ir_test_step();const bridged=state();
    for(const opt of [0,1]){setup_bridge();bridge[opt].exports.f(0);assert.equal(words[664>>2],102);assert.deepEqual(state(),bridged);}
