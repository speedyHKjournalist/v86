...
**游戏负载（KartRider，高压场景存档，2026-09-25）。** `tests/ir/performance/game_state.mjs` 恢复浏览器里用
v86gl 代理保存的状态（内存/显存大小取自状态），图形用“无限快”的空渲染器（应答 D9WG 查询、回读与心跳），
音频用按采样率消费 SB16 DMA 的扬声器，只测客户机 CPU。稳定后 Tier-0 约 270 MIPS、26 fps，legacy 约
190 MIPS、6–9 fps。定位并修掉的调度问题（修之前 Tier-0 只有 7–9 fps）：
- FAST_STAMP 变化后页见证全部失效，只有完整准入路径会重新填写；一旦 `t0_execute` 未命中，精确见证的快路径
  （`fast_execute`）会一直链接下去，页见证再也不会被填。现在每次失效后一次性重填所有已发布页函数的见证。
- `fast_execute` 的链接循环遇到 Tier-0 页函数时交给 `t0_execute` 的链接循环（后者没有逐次激活的记账）。
- 页热度批表只有 64 槽，槽被别的页占用时直接丢掉累计的热度；游戏的解释代码分散在很多页上，热度永远到不了
  调度器。现在换出时先把热度交给原来的页，表扩大到 256 槽（解释执行的指令从 4% 降到 0.2%）。
- 编译时抓取页快照失败（页在当前地址空间未映射）不再把页标成永久失败，之后的热度会再次触发编译。

跨页转移（每秒约 1600 万次，多为成对的调用/返回）随后用**簇函数**处理：页函数可以覆盖不相邻的页
（`analysis::Slots` 把函数偏移的第 k 个 4 KiB 映射到线性页 `pages[k]`，最多 `tier0::MAX_PAGES` = 6 页），
簇外目标以 `far_eip` 退出。选页靠 `cache::note_link`：每个源页（两路组相联表）记录出链总数和
space-saving 计数的前 4 个目标页；某目标保证计数到 10 万且占源页这段窗口内出链的五分之一以上时，
源页带上它重编译（相邻页走 `want_range`，否则 `want_partner`，最多 5 个伙伴），随后计数清零重新统计。
伙伴选择还有三条规则：RET 造成的链接（弹出的地址仍在 ESP 下方且等于目标）只有在调用方页面自己主要走回
本页时才算数——否则分发器会被复制进每个处理函数（502.codebloat 从 1.37× 掉到 0.69×，各份副本还要分别
tier-up）；选中的伙伴若自己主要继续到另一页，沿这条链一并加入（页链一次重编译完成）；伙伴页的代码若一直
执行到下一页，下一页也加入（与范围函数相同的 `tier0::runs_on` 判断）。
第一版用直接映射的页对表：冲突时每次减 64，出链总数又是累计值，预热之后几乎所有热页对都通不过“占五分之一”
的检查，最热的一对（0x404000 的循环每次调用 0x528580 的 4 条指令取值函数）从没进簇。改正后链接次数
从每秒约 1200 万降到约 600 万。另外，页函数不经过 TLB 取指，上下文切换清空 TLB 后，`t0_execute`
先按解释器的方式翻译 EIP（`get_phys_eip`）再查见证。

**踩坑：取指缺页的 EIP。** 上面这次翻译若缺页，异常帧里的 EIP 取自 `previous_ip`；最初没有先把
`previous_ip` 设为 EIP，于是记录的是上一条已退役的指令（SYSEXIT/IRET/CALL 之类的控制转移）。
内核换入页面后 IRET 回去重新执行那条指令：用户态回到内核地址（XP 的 lsass 以 0xC0000005 被结束，
winlogon 60 秒后重启），内核里 NTFS 可换出的代码重复执行 RET/CALL 导致蓝屏 0x24（冷启动三次里三次）。
`tests/ir/differential/tier0_fetch_fault.mjs` 覆盖这种情况（被调用页定期取消映射，比较缺页处理程序看到的
EIP 与栈），修正前的构建在该测试里有 133 次错误 EIP、栈偏移 532 字节。

**帧率与客户机的呈现节流。** 客户机空闲约 17% 并不是 CPU 等待：D3D9 代理的 `Present` 用
`GetTickCount` 按 16 ms 间隔节流，而 XP 的 `GetTickCount` 以 10 ms 为步长，14 ms 的帧被读成 10 ms 又多睡
6 ms——帧时间 13–20 ms 之间一律卡在 50 fps，CPU 再快也到不了 60。代理改为以 `QueryPerformanceCounter`
为时钟、按截止时间排节奏后（`retro-gaming-site/glbridge/d3d9proxy`，`GetRasterStatus` 同样修正），
用内存补丁在存档上验证：簇函数 + 修正后稳定 59–60 fps（120 秒后半均值 59.4，此前 46–48.5）；
不节流时后半均值 73 fps（62–78，帧时间多为 12–14 ms，464 MIPS）。quick 套件 warm 1.67×、cold 1.35×
（`build/bench/results-t0-clusters.json`；708.pages 收敛更慢：quick 下 1.05–1.22×，长跑稳定 1.5×），
XP 启动到桌面 Tier-0 12.4–14.0 s，legacy 15.1–16.5 s（交替测量）。
