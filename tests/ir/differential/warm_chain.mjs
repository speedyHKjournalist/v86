...
const alternate = PC + 0x15000, iterations = 512, initialCount = 0xFFFFFFF0;
...
try {
    ...
    let compared = 0, observedHandoffs = 0;
    for(const kind of ['ordinary', 'guest_smc', 'raw_code', 'notified_code', 'mapping', 'reset', 'xmm',
        ...
        for(const enabled of [0, 1]) {
            ...
            const runCount = (observer ? 9 : kind === 'guest_smc' ? 10 : 8) * iterations + 1
                + (kind === 'guest_smc' ? 1 : 0);
            ...
            resetRegisters(); e.full_clear_tlb();
            const beforeHandoffs = e.ir_cache_stat(35);
            run();
            const handoffs = (e.ir_cache_stat(35) - beforeHandoffs) >>> 0;
            if(!enabled || ['recording', 'diagnostics', 'full_validation'].includes(kind)) {
                ...
                observedHandoffs += handoffs;
            }
            assert.equal(count(), (initialCount + runCount) >>> 0, `${kind}: wrapped retirement`);
            ...
            if(kind === 'diagnostics') {
                ...
                assert.equal(d.totals.ir_steps + d.totals.interpreter_steps + d.totals.legacy_steps, runCount);
            ...
            if(['between_batches', 'replacement'].includes(kind)) {
                ...
                resetRegisters(); cpu.mem8[pages[2]] = 0x4F;
                if(kind === 'replacement') {
                ...
                assert.equal(count(), (initialCount + runCount) >>> 0);
            }
    ...
    assert(observedHandoffs > 0);
    console.log(`PASS: ${wasm}: ${compared} paired warm-chain scenarios, ${observedHandoffs} handoffs; exact wrapped retirement, raw/notified SMC, remapping, reset, XMM, held owner, replacement, host batches and instrumentation fallback`);
