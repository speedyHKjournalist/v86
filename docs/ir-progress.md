...
| IR-09 | 部分基础 | 整数后端可执行 CFG 和寄存器代码；冷 CPU 入口、真实状态 ABI、显式入口键、在线发布/自动升档均已存在。structured CFG 已从第一阶段自然循环扩展到：多块线性自然循环、条件回边/退出两侧的有限 Jump 链、以及单条件分支的 if/else→join→公共 tail diamond；命中后直接生成 Wasm loop/if/branch，保留 parallel-copy、budget/recovery、RAM loop-cache reset；nested branch、多回边、额外 side region 等仍 generic pc-local dispatcher fallback。完整 Tier 1 语义、更一般 reducible CFG structuring、成熟区域选择和系统验收仍未完成，详见 ir09-structured-cfg.md |
