...
剩下的主要瓶颈是**跨页转移**：每秒约 1600 万次（约每 17 条指令一次），每次在 Rust 链接循环里约 12 ns，
加上页函数的出入口，占 CPU 时间约四成。转移高度集中：前 10 对页占 37%、前 200 对（涉及 98 页）占 74%，
而且多是成对的调用/返回（如 0x71e000↔0x59d000）。下一步应当把紧密相连、不相邻的页编进同一个函数
（一般化现在的相邻页多页函数），让调用与返回变成函数内跳转。
