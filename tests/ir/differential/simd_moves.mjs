...
for(const release of [false,true]){
 ...
 try{
    ...
    const chain=extra("chain"),resume=extra("resume"),edge=extra("edge"),chainBytes=JSON.parse(fs.readFileSync("build/ir-simd-moves/chain.json"));
    ...
    for(const [c,i] of selected){
        ...
        compare(i,()=>{device(i);onEvent=()=>{xmm[28]=0x12345678;cpu.reg32[0]=0xCAFEBABE;};});assert.equal(xmm[28],0x12345678);assert.equal(cpu.reg32[0]>>>0,0xCAFEBABE);mutations++;
        if(c[5]){
    ...
    const baseCase=cases.findIndex(c=>c[1]&&c[2]===32&&c[3]===0x0F10&&c[6]===0&&c[7]===8);
    const setupChain=(hot,deviceLoad=false)=>{reset(baseCase);cpu.segment_offsets.fill(0,0,6);cpu.reg32[6]=BASE+0x40;cpu.reg32[7]=BASE+0x10040;mem.fill(0xCC,BASE+0x10040,BASE+0x10050);mem.set(chainBytes,PC);e.full_clear_tlb();if(hot){e.ir_memory_write(BASE+0x40,mem[BASE+0x40],1);e.ir_memory_write(BASE+0x10040,mem[BASE+0x10040],1);}if(deviceLoad){set32(0x13000+0x310*4,0xA0007);e.full_clear_tlb();onEvent=kind=>{if(kind.startsWith("r"))xmm[24]=0xDEADBEEF;};}slow=guards=0;events=[];};
    let chains=0;
    for(const hot of [false,true])for(const deviceLoad of [false,true]){
        const steps=hot&&!deviceLoad?5:3;setupChain(hot,deviceLoad);for(let n=0;n<steps;n++)e.ir_test_step();const expected=state(),observed=events.slice();for(let n=steps;n<5;n++)e.ir_test_step();const completed=state(),out=Buffer.from(mem.slice(BASE+0x10040,BASE+0x10050));
        for(const opt of [0,1]){setupChain(hot,deviceLoad);chain[opt].exports.f(0);assert.equal(words[664>>2],100+steps);assert.deepEqual(state(),expected);assert.deepEqual(events,observed);if(steps===3)resume[opt].exports.f(0);assert.equal(words[664>>2],105);assert.deepEqual(state(),completed);assert.deepEqual(Buffer.from(mem.slice(BASE+0x10040,BASE+0x10050)),out);chains++;}
    }
    ...
    for(const opt of [0,1]){reset(baseCase);const expected=Array.from(xmm);expected.splice(0,8,...expected.slice(4,8),...expected.slice(0,4));edge[opt].exports.f(0);assert.deepEqual(Array.from(xmm),expected);assert.equal(words[664>>2],101);}
