...
fn fixture() -> Fixture {
    ...
    Fixture { region: b.region, entry, header, body, input, flags, invariant, variant: next_acc }
}
...
fn licm_moves_only_invariants_and_preserves_recovery_graph() {
    ...
    let edges = format!("{:?}", f.region.blocks.iter().map(|b| (&b.params, &b.terminator, b.entry_state)).collect::<Vec<_>>());
    ...
    assert_eq!(edges, format!("{:?}", f.region.blocks.iter().map(|b| (&b.params, &b.terminator, b.entry_state)).collect::<Vec<_>>()));
    verify(&f.region).unwrap();
...
fn licm_budget_failure_is_atomic_even_after_planned_moves() {
    ...
    for config in [
        Config { max_hoists: 1, ..Config::default() },
...
fn licm_does_not_create_preheaders_or_change_branch_budgeting() {
    let mut f = fixture();
    let Terminator::Branch(edge) = f.region.blocks[f.entry.index()].terminator.take().unwrap() else {
        panic!()
    };
    ...
    let condition = f.region.append(f.entry, Op::Const(1), vec![], &[Type::I1], None)[0];
    f.region.terminate(f.entry, Terminator::CondBranch {
        condition,
        taken: edge,
        not_taken: Edge { target: cold, args: vec![] },
    });
    verify(&f.region).unwrap();
    let before = format!("{:?}", f.region);
    let stats = run(&mut f.region, Config::default()).unwrap();
...
fn licm_keeps_cpu_reads_and_snapshot_observations_in_place() {
    ...
    let read = f.region.append(f.body, Op::ReadGpr(6), vec![], &[Type::I32], None)[0];
    let dependent = f.region.append(f.body, Op::Binary(Binary::Add), vec![read, f.input[3]], &[Type::I32], None)[0];
...
fn licm_speculation_whitelist_excludes_every_stateful_class() {
    let template = Instruction {
        block: BlockId(0), op: Op::Const(7), args: vec![], results: vec![ValueId(0)],
        state: None, commit: None, trap_after_fault: false, unmasked_word_store: false,
    };
    ...
    for op in [
        ...
        Op::Divide { bits: 32, signed: false }, Op::SegmentAddress { segment: 3 },
        Op::PopAddress { segment: 2, bytes: 4 }, Op::GuestLoad { bytes: 4 },
        Op::GuestStore { bytes: 4 }, Op::PartialStore { bytes: 4 },
        Op::GuestCheck { bytes: 4, write: false }, Op::CallHelper(HelperId(0)),
        Op::RmwLoad { bytes: 4, order: RmwOrder::Plain },
        Op::RmwStore { bytes: 4, order: RmwOrder::Locked },
        Op::CompareExchange8B { order: RmwOrder::Plain }, Op::PollBudget, Op::SseCheck,
        Op::XmmLoad { bytes: 16, register: 0 },
        Op::XmmStore { lane: 0, bytes: 16, register: 0 },
        Op::XmmMaskedStore { source: 0, mask: 1 },
    ] {
        assert!(!movable(&Instruction { op, ..template.clone() }));
    }
    assert!(!movable(&Instruction { state: Some(StateId(0)), ..template.clone() }));
    assert!(!movable(&Instruction { commit: Some(StateId(0)), ..template.clone() }));
    assert!(!movable(&Instruction { trap_after_fault: true, ..template.clone() }));
    assert!(!movable(&Instruction { unmasked_word_store: true, ..template }));
}
...
fn graph(edges: &[&[usize]]) -> Region {
    ...
    for i in 0..edges.len() { r.block(i == 0); }
    for (b, successors) in edges.iter().enumerate() {
        let edge = |i: usize| Edge { target: BlockId(i as u32), args: vec![] };
        let term = match *successors {
            [] => Terminator::Exit(StateId(0)),
            [a] => Terminator::Branch(edge(*a)),
            [a, b] => Terminator::CondBranch { condition: ValueId(0), taken: edge(*a), not_taken: edge(*b) },
...
fn loop_analysis_excludes_irreducible_cycles_and_external_entries() {
    assert!(analyze(&graph(&[&[1, 2], &[2, 3], &[1, 3], &[]])).loops.is_empty());
    let mut r = graph(&[&[1], &[1, 2], &[]]);
    r.entries.push(BlockId(1));
    assert_eq!(analyze(&r).loops[0].preheader, None);
    let a = analyze(&graph(&[&[1], &[1, 2], &[], &[1]]));
    assert_eq!(a.loops[0].preheader, Some(BlockId(0)), "dead incoming edges are not external entries");
    let a = analyze(&graph(&[&[1], &[2], &[1, 1]]));
    assert_eq!(a.loops[0].latches, vec![BlockId(2)], "duplicate edges do not duplicate latches");
...
fn licm_wasm_execution_fixtures() {
    ...
    for budget in [1, 2, 3, 4, 5, 8, 9, 16, 64, 100] {
        for optimize in [false, true] {
            let mut f = fixture();
            if optimize { assert_eq!(run(&mut f.region, Config::default()).unwrap().hoisted, 6); }
            let artifact = emit(&lower(&f.region).unwrap(), layout(), budget).unwrap();
            std::fs::write(format!("build/ir-licm/loop-{budget}-{optimize}.wasm"), artifact.bytes).unwrap();
        }
    }
}
