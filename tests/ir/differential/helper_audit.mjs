...
for(const family of families) {
    ...
    let modules=0,semanticModules=0,totalBytes=0;
    const imports={};
    for(let i=0;i<manifest.length;i++) {
        ...
        modules++;totalBytes+=bytes.length;
        const semantic=list.filter(x=>/^ir_(sse_fp|mmx)/.test(x.name));
        if(semantic.length)semanticModules++;
    ...
    report.families[family]={modules,semanticModules,averageBytes:Math.round(totalBytes/modules),imports};
}
