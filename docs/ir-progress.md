...
| IR-10 | 部分基础 | 有界常量折叠、支配关系 GVN、trivial phi 消除、StateMap-aware DCE、常量分支裁剪和保留预算检查的直线块合并；已有独立 MIR 字面量常量折叠；新增宽度感知的标量恒等式、子寄存器往返化简及交换律规范化；其余跨块 FLAGS/状态同步优化未完成 |
| IR-11 | 部分实现 | 新增有界、事务式纯 SSA LICM（已有前置块、多回边、嵌套循环）、SIMD lane/shuffle 化简；不移动访存、CPU 读取、守卫或预算点。proof-based 访存复用、forwarding、其余循环/SIMD 优化仍未实现 |
...
## 后续推进：有界纯 SSA 循环优化与值规范化

- 新增 `PassConfig.canonicalize` 与 `PassConfig.licm`，接入现有优化管线及
  Tier 2 默认 pass 配置；不改变生产默认 legacy 后端，也不使 Tier 1 自动执行这些 pass。
- 标量规范化按实际机器移位掩码及 SSA 位宽处理恒等式、比较、select、
  子寄存器 extract/insert 和 extend/truncate；所有别名同步更新恢复状态和 CFG 边参数。
- SIMD 化简保留窄 lane 高位截断语义，只组合最多两个底层向量源的 shuffle；
  三源情况及不满足对齐条件的提取保持原状。没有引入浮点或 x87 快速数学假设。
- LICM 使用显式纯操作白名单、支配关系和现有无条件前置块。规划预算失败时
  保留原 Region，修改后的候选先通过 verifier 再提交；不减少预算出口或重排异常点。
- 新增 15 项 Rust 测试；一次完整 warnings-as-errors 调用通过 142 项测试。
  独立 oracle 通过 12,150 次循环预算执行、57,024 次标量执行和 16,380 次 SIMD 执行。
- 这是 IR-10/IR-11 的增量实现，不代表 IR-00～IR-14 完成；覆盖清单、
  默认切换 gate、剩余 ISA、访存 proof、系统/XP/性能验收及旧 emitter 退役条件均不变。

详见 [循环与值优化说明](ir-loop-optimizations.md)。

IR 差分框架另将两个大型 SIMD corpus 改为 32 对实例的有界 LRU 缓存，
避免同时保活数万 Wasm code spaces；没有减少 case、模式或断言。
