...
fn counted_loop(poll: bool) -> (Region, [ValueId; 3], BlockId) {
    ...
    if poll {
        ...
        effect = b
            .region
            .append(body, Op::PollBudget, vec![effect], &[Type::Effect], Some(s))[0];
    }
    ...
    b.region.terminate(
        ...
        Terminator::Branch(Edge {
            target: header,
            args: vec![effect, next_counter, next_sum],
        }),
    );
    b.block = exit;
    b.effect = xe;
...
fn graph(edges: &[&[usize]], entries: &[usize]) -> Region {
    ...
    for (i, targets) in edges.iter().enumerate() {
        ...
        let edge = |n: usize| Edge {
            target: BlockId(n as u32),
            args: vec![],
        };
        r.terminate(
            b,
...
fn mutable_cpu_reads_faults_and_effects_are_not_speculation_candidates() {
    ...
    let Definition::Instruction(id, _) = r.values[values[0].index()].definition else {
        panic!()
    };
    ...
    i = template.clone();
    i.commit = Some(StateId(0));
    assert!(!speculatable(&i));
    i = template.clone();
    i.trap_after_fault = true;
    assert!(!speculatable(&i));
    i = template;
    i.unmasked_word_store = true;
    assert!(!speculatable(&i));
}
    ...
    for poll in [false, true] {
        for budget in [1, 2, 3, 4, 5, 8, 16, 32, 100] {
            for optimized in [false, true] {
                ...
                std::fs::write(
                    format!("build/ir-licm/loop-{poll}-{budget}-{optimized}.wasm"),
                    bytes,
                )
                .unwrap();
            }
