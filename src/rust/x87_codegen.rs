...
use crate::cpu::{global_pointers, memory};
...
fn decode(address: u32) -> Option<Op> {
    ...
    if opcode == 0xD9 {
        return match byte {
            0xC0..=0xC7 => Some(Op::Push(r)),
            0xC8..=0xCF => Some(Op::Exchange(r)),
            0xFC => Some(Op::Round),
            0xD0 => Some(Op::Nop),
            0xE0 | 0xE1 => Some(Op::Unary(byte == 0xE0)),
            0xE8 | 0xEE => Some(Op::Constant(byte == 0xE8)),
            0xF6 | 0xF7 => Some(Op::Rotate(byte == 0xF7)),
            _ => None,
        };
    }
    if matches!(opcode, 0xDB | 0xDF) && matches!(byte & 0xF8, 0xE8 | 0xF0) {
        return Some(Op::Compare {
            r,
            pops: (opcode == 0xDF) as usize,
            integer: true,
            quiet: byte & 0xF8 == 0xE8,
        });
    }
    ...
    if opcode == 0xDD {
        return match byte & 0xF8 {
            0xC0 => Some(Op::Free(r)),
            0xD0 | 0xD8 => Some(Op::Store {
                r,
                pop: byte & 8 != 0,
            }),
            0xE0 | 0xE8 => Some(Op::Compare {
                r,
                pops: (byte & 8 != 0) as usize,
                integer: false,
                quiet: true,
    ...
    if !matches!(opcode, 0xD8 | 0xDC | 0xDE) {
        return None;
    }
    let group = (byte >> 3) & 7;
    let op = match group {
    ...
    Some(Op::Arithmetic {
        op,
        r,
        reverse: group == 5 || group == 7,
        target_r: opcode != 0xD8,
        pop: opcode == 0xDE,
    })
}
...
pub fn try_region(ctx: &mut JitContext) -> bool {
    ...
    if start >= ctx.last_instruction_in_block {
        return false;
    }
    if !matches!(memory::read8(start), 0xD8..=0xDF) {
        return false;
    }
    let mut plan = Plan::default();
    let mut end = start;
    while end + 2 <= ctx.last_instruction_in_block && plan.ops.len() < 32 {
        let Some(op) = decode(end)
        else {
            break;
        };
    ...
    for op in &plan.ops {
        match *op {
            Op::Arithmetic {
                ...
                ctx.builder
                    .set_local_f64(locals[if target_r { r } else { top }].as_ref().unwrap());
                if pop {
                    top = (top + 1) & 7;
                }
            },
            Op::Compare {
                ...
                emit_compare(
                    ctx,
                    locals[top].as_ref().unwrap(),
                    locals[(top + r) & 7].as_ref().unwrap(),
                    integer,
                    quiet,
                );
                top = (top + pops) & 7;
            ...
            Op::Store { r, pop } => {
                ...
                ctx.builder
                    .set_local_f64(locals[(top + r) & 7].as_ref().unwrap());
                if pop {
                    top = (top + 1) & 7;
                }
            },
            ...
            Op::Constant(one) => {
                top = (top + 7) & 7;
                ctx.builder
                    .const_i64(if one { 0x3FF0000000000000 } else { 0 });
                ctx.builder.reinterpret_i64_as_f64();
                ctx.builder.set_local_f64(locals[top].as_ref().unwrap());
            },
            Op::Rotate(increment) => {
                top = (top + if increment { 1 } else { 7 }) & 7;
            },
            Op::Free(_) | Op::Nop => {},
            Op::Exchange(r) => {
                let r = (top + r) & 7;
            ...
            Op::Push(r) => {
                ctx.builder
                    .get_local_f64(locals[(top + r) & 7].as_ref().unwrap());
                top = (top + 7) & 7;
                ctx.builder.set_local_f64(locals[top].as_ref().unwrap());
            },
        }
    ...
    for (r, tag) in plan.tags.iter().enumerate() {
        if let Some(full) = tag {
            metadata |= 1 << (r + if *full { 12 } else { 4 });
        }
    }
    let counts = plan
        .counts
        .iter()
        .enumerate()
        .fold(0, |bits, (op, count)| bits | (count << (op * 8)));
    ctx.builder
        .const_i32((plan.dirty | (plan.comparisons << 8)) as i32);
    ...
    for local in locals.into_iter().flatten() {
        ctx.builder.free_local_f64(local);
    }
    ctx.builder.else_();
...
pub fn prepare_instruction(ctx: &mut JitContext) {
    ...
    while matches!(
        memory::read8(a),
        0x66 | 0x67 | 0xF2 | 0xF3 | 0x26 | 0x2E | 0x36 | 0x3E | 0x64 | 0x65
    ) {
        let byte = memory::read8(a);
        if matches!(byte, 0x66 | 0xF2 | 0xF3) {
            simd_prefix = byte;
        }
        a += 1;
        if a - start >= 15 {
    ...
    if observes_mmx {
        ctx.builder.call_fn0("fpu_cache_barrier");
    }
}
