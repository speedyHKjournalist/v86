...
## 2026-09-24：XP 性能评审与热度/代码体积修正

- 热度改为按退休指令加权的带标签直接映射表（默认 32 × 4096 = 131072 条指令），
  消除 128 项 FIFO 环的热度丢失；缓存默认容量 768，XP 启动零驱逐。
- 非融合区域使用精确稀疏预算（入口前沿与循环头按最长路径扣费，退休数仍受预算约束），
  区域级 backing 事实数据流在 Tier-1 启用：XP 重放语料的 wasm 体积 −42%。
- 端口 I/O 通知观察者、链内退休见证、Tier-2 选伴与近期升档、TLB 基址固定槽位、
  融合默认关闭。
- 同步磁盘 XP 里程碑：IR 20.7 s → 15.6～15.8 s，legacy 13.1～13.8 s；异步：
  IR 26.7 s → 20.9～21.1 s，legacy 19.0～19.6 s。**仍未超过 legacy**，剩余差距为每次
  激活开销、按入口编译导致的覆盖率与编译成本。详见
  [本轮评审](ir-performance-review-20260924.md)。
