...
| IR-09 | 部分基础 | 整数后端可执行 CFG 和寄存器代码；冷 CPU 入口、真实状态 ABI、显式入口键、在线发布/自动升档均已存在。新增第一阶段 structured CFG emitter：single-entry 自然循环及“一侧回边、一侧退出”的简单条件循环直接生成 Wasm loop/branch，保留 parallel-copy、budget/recovery、RAM loop-cache reset；其他图继续 generic pc-local dispatcher fallback。完整 Tier 1 语义、更多 reducible CFG structuring、成熟区域选择和系统验收仍未完成，详见 ir09-structured-cfg.md |
...
| IR-13 | 完整矩阵未完成 | host/browser/device acceptance gate 与同核 execution-budget 矩阵已建立。#45 的 exact target-entry 数据显示 128/256/512/1024/2048/4096 budget 的目标热循环分别退休 127/255/511/1023/2047/4095 条 guest 指令/activation，而 4096 throughput 约 766k steps/ms、仅为 paired legacy 的约 60%，说明 outer activation 开销显著但不足以解释全部差距；下一步通过 IR-09 structured CFG emission 直接消除 region 内 pc-local dispatcher，并重跑同一矩阵。XP、真实应用/游戏、稳定冷热性能阈值及剩余 IR-02～IR-09 回补仍未完成 |
