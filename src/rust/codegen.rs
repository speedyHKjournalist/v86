...
pub fn gen_lookup_current_module_target(
    ctx: &mut JitContext,
    flags: crate::state_flags::CachedStateFlags,
) {
    ...
    let cached = ctx
        .target_caches
        .get(&ctx.start_of_current_instruction)
        .map(|(key, epoch, state)| {
            (
                key.unsafe_clone(),
                epoch.unsafe_clone(),
    ...
    if let Some((key, epoch, state)) = cached.as_ref() {
        ...
        ctx.builder.eq_i32();
        ctx.builder
            .load_fixed_i64(std::ptr::addr_of!(crate::jit::CODE_LOOKUP_EPOCH) as u32);
    ...
    ctx.builder.shl_i32();
    ctx.builder
        .load_aligned_i32(std::ptr::addr_of!(tlb_code) as u32);
...
pub fn clear_ram_read_cache(ctx: &mut JitContext) {
    if let Some(local) = ctx.ram_read_cache.take() {
        ctx.builder.free_local(local);
    }
    if let Some(local) = ctx.ram_read_base.take() {
        ctx.builder.free_local(local);
    }
...
fn peek_ram_read_gap(
    ...
    let writes = match op {
        ...
        | 0x85 | 0x89 | 0x8B => {
            ...
            if matches!(op, 0x39 | 0x3B | 0x85) {
                0
            }
            else {
                1 << (if op & 2 != 0 { (m >> 3) & 7 } else { m & 7 })
            }
        },
        0x81 | 0x83 => {
            let m = cpu.read_imm8();
            if m < 0xC0 || matches!((m >> 3) & 7, 2 | 3) {
                return None;
            }
            if op == 0x81 {
                cpu.read_imm32();
    ...
    let arithmetic = if !unsafe { crate::jit::JIT_EXTENDED_FLAGS } {
        ...
        !word
            && match op {
                ...
                0x05 | 0x2D => true,
                0x81 | 0x83 => {
                    memory::read8(a + 1) >= 0xC0 && matches!(memory::read8(a + 1) >> 3 & 7, 0 | 5)
                },
                _ => false,
            }
    }
...
fn peek_memory_run(
    ...
    if if write {
        matches!(op, 0x50..=0x57 | 0x68 | 0x6A | 0xE8)
    }
    else {
        matches!(op, 0x58..=0x5F | 0xC2 | 0xC3)
    } {
        if !unsafe { crate::jit::JIT_STACK_CACHE } {
            return None;
        }
        if op == 0x68 || op == 0xE8 {
            if word {
    ...
    if !write {
        ...
        return if operand.uses_register_mask((1 << regs::ESP) | (1 << regs::EBP)) {
            Some(next)
        }
        else {
            None
    ...
    if word || !unsafe { crate::jit::JIT_RMW_CACHE } {
        return None;
    }
...
fn prepare_memory_run(ctx: &mut JitContext, write: bool) -> bool {
    if !unsafe { crate::jit::JIT_STACK_CACHE || crate::jit::JIT_RMW_CACHE } {
        return false;
    }
    if !ctx.cpu.state_flags.is_32()
        || !ctx.cpu.has_flat_segmentation()
    ...
    let Some(mut next) = peek_memory_run(ctx.cpu, write)
    else {
        return false;
    };
    let mut keep = false;
    for _ in 0..=8 {
        if next.eip > ctx.last_instruction_in_block || crate::jit::is_near_end_of_page(next.eip) {
            break;
        }
        if peek_memory_run(&next, write).is_some() {
            keep = true;
            break;
        }
        if let Some((_, after)) = peek_ram_read_gap(&next) {
            next = after;
        }
        else {
            break;
...
pub fn prepare_ram_read(ctx: &mut JitContext) {
    if prepare_memory_run(ctx, false) {
        return;
    }
...
pub fn clear_ram_write_cache(ctx: &mut JitContext) {
    if let Some(local) = ctx.ram_write_cache.take() {
        ctx.builder.free_local(local);
    }
    if let Some(local) = ctx.ram_write_base.take() {
        ctx.builder.free_local(local);
    }
...
fn peek_integer_write(
    ...
    let byte = cpu.read_imm8();
    if byte >= 0xC0 || matches!(op, 0xC6 | 0xC7) && byte & 0x38 != 0 {
        return None;
    }
    let operand = modrm::decode(&mut cpu, byte);
    match op {
...
pub fn prepare_ram_write(ctx: &mut JitContext) {
    if prepare_memory_run(ctx, true) {
        return;
    }
    if ctx.ram_write_active && ctx.ram_write_continue && peek_ram_read_gap(ctx.cpu).is_some() {
        return;
    }
...
fn gen_safe_read(
    ...
    if ctx.ram_read_dynamic {
        ...
        if let Some(local) = ctx.ram_read_base.as_ref() {
            ...
            ctx.ram_read_base = Some(ctx.builder.set_new_local());
        }
    }
    ctx.builder.block_end();
...
fn gen_safe_write(
    ...
    if ctx.ram_write_active {
        ...
        ctx.builder.const_i32(!0xFFF);
        ctx.builder.and_i32();
        if let Some(base) = ctx.ram_write_base.as_ref() {
            ctx.builder.set_local(base);
        }
        else {
            ctx.ram_write_base = Some(ctx.builder.set_new_local());
        }
    }
    ctx.builder.block_end();
    ...
    if ctx.ram_write_continue {
        ctx.ram_write_cache = Some(entry_local);
    }
    else {
        ctx.builder.free_local(entry_local);
        clear_ram_write_cache(ctx);
...
pub fn gen_store_flag_value(builder: &mut WasmBuilder, address: u32) {
    if builder.defer_flags {
        builder.defer_fixed_i32(address);
    }
    else {
...
pub fn gen_load_flag_value(builder: &mut WasmBuilder, address: u32) {
    if !builder.load_deferred_i32(address) {
        builder.load_fixed_i32(address);
    }
}
...
pub fn gen_fpu_get_sti(ctx: &mut JitContext, i: u32) {
    ...
    ctx.builder.load_aligned_i64(0);
    ctx.builder.get_local(&address);
    ctx.builder.load_aligned_u16(8);
    ctx.builder.free_local(address);
...
pub fn gen_fpu_push(ctx: &mut JitContext) {
    ...
    ctx.builder.const_i32(!0x200);
    ctx.builder.and_i32();
    ctx.builder.store_aligned_u16(0);
    ctx.builder.get_local(&top);
    ctx.builder.call_fn1("fpu_invalidate_slot");
    ctx.builder.get_local(&top);
    ctx.builder.const_i32(4);
    ctx.builder.shl_i32();
    ctx.builder.const_i32(global_pointers::fpu_st as i32);
    ...
    ctx.builder.store_aligned_i64(0);
    ctx.builder.get_local(&address);
    ctx.builder.get_local(&exponent);
    ctx.builder.store_aligned_u16(8);
    ctx.builder.free_local(address);
    ctx.builder.else_();
    ctx.builder.get_local_i64(&mantissa);
    ctx.builder.get_local(&exponent);
    ...
    ctx.builder.free_local(exponent);
}
...
pub fn gen_helper_registers_after(ctx: &mut JitContext, name: &str, register: Option<u32>) {
    ...
    for i in 0..8 {
        ...
        ctx.builder
            .load_fixed_i32(global_pointers::get_reg32_offset(i));
