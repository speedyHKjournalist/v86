...
const cases=JSON.parse(fs.readFileSync("build/ir-sti/cases.json"));
const modules=cases.map((_,i)=>[0,1,2,3].map(opt=>new WebAssembly.Module(fs.readFileSync(`build/ir-sti/${i}-${opt}.wasm`))));
...
for(const release of [false,true]){
    ...
    try {
        ...
        while(view.getUint16(0x500,true)!==0xCAFE){
            assert(performance.now()<deadline,"STI shadow guest bootstrap timed out");
            await sleep(1);
        }
        await vm.stop();

        controlled=true;
        const PC=0x8000,STACK=0x90000,UD=0x180100,NM=0x180300,PF=0x180400,GP=0x180500,IRQ=0x180600,INT=0x180700,DATA=0x6000;
        ...
        function state(){
            return {
                ...
                cr2:cpu.cr[2]>>>0,
                sreg:Array.from(cpu.sreg),segments:Array.from(cpu.segment_offsets),cpl:linear32[612>>2],halt:cpu.in_hlt[0],
        ...
        function reset(i,{task=0,empty=0,top=0,flags=0x8D7,delta=0,pageFault: page_fault=false,nullSegment: null_segment=false,mmio=false,sample=0,rounding=0,pending=false,cpl=0,iopl=0,initialIf: initial_if=false}={}){
            ...
            desc(1,0,0x9B|cpl<<5);desc(2,0,0x93|cpl<<5);
            cpu.sreg[1]=8|cpl;cpu.sreg[2]=16|cpl;
            cpu.gdtr_offset[0]=0x3000;cpu.gdtr_size[0]=23;
            cpu.idtr_offset[0]=0x2000;cpu.idtr_size[0]=0x7FF;
            for(const [vector,handler] of [[3,INT],[6,UD],[7,NM],[13,GP],[14,PF],[0x20,IRQ]]){
                set32(0x2000+vector*8,(8|cpl)<<16|handler&65535);
                set32(0x2004+vector*8,handler&0xFFFF0000|0xEE00);
            }
            set32(0x12000,0x13007);
            for(const page of [0,2,3,6,7,8,0x18,0x8F,0x90]){
                set32(0x13000+page*4,page*4096|7);
            }
            e.full_clear_tlb();
        ...
        function compare(i,configure,expected_count){
            ...
            assert.equal(linear32[664>>2],100+(expected.ip===GP?0:cases[i][5]),JSON.stringify({i,ip:expected.ip,cpl:expected.cpl,cr2:expected.cr2}));
            for(const opt of [0,1,2,3]){
                configure();
                instances[i][opt].exports.f(0);
                assert.equal(linear32[664>>2],expected_count);
                assert.deepEqual(state(),expected,`STI shadow case ${i}/${opt}`);
            }
            return expected;
        }
        ...
        for(let i=0;i<cases.length;i++) {
            ...
            if(memory) for(const mmio of [false,true]) {
                compare(i,()=>reset(i,{mmio,delta:0xFFF,pending:true}),102+depth);comparisons++;
                const result=compare(i,()=>reset(i,{delta:0xFFF,pageFault:true,pending:true}),101+depth);
                assert.equal(result.ip,PF);comparisons++;
            }
            if(name==="sse") for(const task of [4,8,12]) {
                const result=compare(i,()=>reset(i,{task,pending:true}),101+depth);
                assert.equal(result.ip,task===8?NM:UD);comparisons++;
            }
            // Same-privilege exception gates keep this check independent of TSS setup.
            for(const cpl of [1,3]) for(let iopl=0;iopl<cpl;iopl++) {
                const result=compare(i,()=>reset(i,{cpl,iopl,pending:true}),101);
                assert.equal(result.ip,GP);comparisons++;
            }
        }
        console.log(`PASS (${release?"release":"debug"}): ${comparisons} STI/nested shadow, pending IRQ, CLI/HLT, branches, memory/FP faults, privilege and one-unit-budget cases in four IR variants`);
