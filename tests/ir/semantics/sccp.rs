...
fn diamond(a: u32, c: u32, forced: Option<bool>, parallel: bool) -> Region {
    ...
    let (taken, not_taken) = if parallel {
        ...
        (Edge { target: join, args: vec![b.effect, x] },
         Edge { target: join, args: vec![b.effect, y] })
    } else {
        ...
        for (block, value) in [(left, a), (right, c)] {
            ...
            b.region.terminate(block, Terminator::Branch(Edge {
                target: join, args: vec![e, value],
            }));
        }
        (Edge { target: left, args: vec![b.effect] },
         Edge { target: right, args: vec![b.effect] })
    };
    b.region.terminate(entry, Terminator::CondBranch { condition, taken, not_taken });
    b.block = join;
    b.effect = effect;
    b.gpr[0] = incoming;
    ...
    b.region.terminate(join, Terminator::CondBranch {
        condition,
        taken: Edge { target: yes, args: vec![effect] },
        not_taken: Edge { target: no, args: vec![effect] },
    });
...
fn loop_region(changing: bool) -> Region {
    ...
    b.region.terminate(b.block, Terminator::Branch(Edge {
        target: header, args: vec![b.effect, zero, seven],
    }));
    b.block = header;
    b.effect = effect;
    b.gpr[0] = count;
    ...
    b.region.terminate(header, Terminator::CondBranch {
        condition,
        taken: Edge { target: header, args: vec![effect, next_count, next_value] },
        not_taken: Edge { target: exit, args: vec![effect, next_count, next_value] },
    });
    b.block = exit;
    b.effect = end_effect;
    b.gpr[0] = end_count;
...
fn config() -> PassConfig {
    PassConfig { merge: false, gvn: false, dce: false, rounds: 1, ..PassConfig::default() }
}
fn conditions(r: &Region) -> usize {
    r.blocks.iter().filter(|b| matches!(b.terminator, Some(Terminator::CondBranch { .. }))).count()
}

#[test]
fn lattice_is_monotone_and_merges_equal_literals() {
    let values = [Lattice::Unknown, Lattice::Constant(0), Lattice::Constant(7), Lattice::Overdefined];
    for &a in &values {
...
fn equal_phi_literals_fold_flags_and_branch_without_rewriting_entry_state() {
    ...
    assert!(stats.constants > 4, "CMP flag expressions should fold through the phi");
    ...
    let state = r.blocks[1].entry_state.unwrap();
    assert_eq!(r.states[state.index()].gpr[0], param);
    assert!(matches!(r.values[param.index()].definition, Definition::Parameter(_, _)));
    verify(&r).unwrap();
}
...
fn loop_carried_constants_widen_when_the_backedge_changes_them() {
    for changing in [false, true] {
        ...
        assert_eq!(analysis.values[invariant.index()], if changing {
            Lattice::Overdefined
        } else {
            Lattice::Constant(7)
        });
...
fn independent_entry_parameters_are_always_unknown_runtime_inputs() {
    ...
    r.terminate(a, Terminator::Branch(Edge { target: b, args: vec![seven] }));
    let one = r.append(b, Op::Const(1), vec![], &[Type::I32], None)[0];
    let result = r.append(b, Op::Binary(Binary::Add), vec![input, one], &[Type::I32], None)[0];
    let flag = r.append(b, Op::Const(0), vec![], &[Type::I1], None)[0];
    let flags = FlagState { arithmetic: [flag; 6], system: one, last_op1: None, raw_zero: None, zero_is_lazy: None };
...
fn ordered_helpers_and_unused_arena_slots_remain_safe() {
    ...
    for owner in [ExceptionOwner::CannotFault, ExceptionOwner::Caller, ExceptionOwner::Helper] {
        ...
        r.append(BlockId(0), Op::Binary(Binary::Add), vec![a, a], &[Type::I32], None);
        ...
        let call = r.instructions.iter().find(|i| matches!(i.op, Op::CallHelper(_))).unwrap().clone();
        let state = format!("{:?}", r.states[call.state.unwrap().index()]);
        assert_eq!(run(&mut r, DEFAULT_WORK_LIMIT).unwrap().constants, 1);
        let after = r.instructions.iter().find(|i| matches!(i.op, Op::CallHelper(_))).unwrap();
    ...
    for (name, original) in cases {
        for mode in ["before", "sccp", "pipeline"] {
            ...
            for budget in [1, 2, 3, 4, 8, 100] {
                ...
                std::fs::write(format!("build/ir-sccp/{name}-{mode}-{budget}.wasm"), bytes).unwrap();
            }
        }
    }
}
