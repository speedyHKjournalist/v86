...
const corpus=JSON.parse(fs.readFileSync("build/ir-simd-integer/cases.json"));
const required=corpus.findIndex(c=>c[1]&&c[2]===32&&c[3]===0x660FFC&&c[6]===0&&c[7]===8);
const {cases,indices}=shardCorpus(corpus,import.meta.url,required);
const modules=indices.map(i=>[0,1].map(opt=>new WebAssembly.Module(fs.readFileSync(`build/ir-simd-integer/${i}-${opt}.wasm`))));
