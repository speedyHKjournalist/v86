...
| IR-09 | 部分基础 | 整数后端可执行 CFG 和寄存器代码；冷 CPU 入口、真实状态 ABI、显式入口键、在线发布/自动升档均已存在。structured CFG 已扩展到：按真实回边落点识别的多块 preheader→natural-loop header、条件回边/退出两侧有限 Jump 链、单条件 if/else→join→公共 tail diamond，以及优化后退化的 Jump-only/单 Exit linear CFG；命中后直接生成 Wasm loop/if/branch，保留 parallel-copy、budget/recovery、RAM loop-cache reset；nested branch、多回边、额外 side region 等仍 generic pc-local dispatcher fallback。完整 Tier 1 语义、更一般 reducible CFG structuring、成熟区域选择和系统验收仍未完成，详见 ir09-structured-cfg.md |
...
| IR-13 | 完整矩阵未完成 | host/browser/device acceptance gate 与同核 execution-budget 矩阵已建立。#46 在不改变 activation policy 的情况下移除目标热循环内部 pc-local dispatcher；IR-core run 362 的 128/256/512/1024/2048/4096 样本全部确认 structured，1024/2048/4096 的 hosted synthetic-loop 吞吐分别约为 paired legacy 的 112%/128%/144%，证明 generic CFG dispatch 是该形态的重要瓶颈，但这不是 XP/应用性能结论。当前继续用 acceptance matrix 扩大 structured CFG 覆盖，并保留 XP、真实应用/游戏、稳定冷热性能阈值及剩余 IR-02～IR-09 回补为未完成项 |
