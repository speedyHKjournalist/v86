...
for(const release of [false,true]){
 ...
 try{
    ...
    const instances=boundedInstances(i=>[0,1].map(opt=>new WebAssembly.Instance(new WebAssembly.Module(fs.readFileSync(`build/ir-simd-integer/${i}-${opt}.wasm`)),{e:imports})));
    ...
    let countsChecked=0,narrows=0;
    const verifyPacked=(c,before,data,after)=>{
        const expected=before.xmm.slice(),reg=c[6],operand=c[7];
        expected.splice(reg*4,4,...packed(c[3],before.xmm.slice(reg*4,reg*4+4),operand<8?before.xmm.slice(operand*4,operand*4+4):data));
    ...
    for(let i=0;i<cases.length;i++){
        ...
        if([0xD1,0xD2,0xD3,0xE1,0xE2,0xF1,0xF2,0xF3].includes(c[3]&255)){
            ...
            for(const high of [0,0xFFFFFFFF])for(const hot of c[7]<8?[false]:[false,true]){
                ...
                },{check:(before,data,after)=>verifyPacked(c,before,data,after)});countsChecked++;
            }
        }
        if([0x63,0x67,0x6B].includes(c[3]&255)){
            ...
            for(let seed=0;seed<edges.length;seed++)for(const hot of c[7]<8?[false]:[false,true]){
                ...
                },{check:(before,data,after)=>verifyPacked(c,before,data,after)});narrows++;
            }
        }
    }
    console.log(`PASS (${release?"release":"debug"}): ${countsChecked} full-u64 count/high-half cases and ${narrows} signed narrowing boundaries`);
    ...
    const baseCase=cases.findIndex(c=>c[1]&&c[2]===32&&c[3]===0x660FFC&&c[6]===0&&c[7]===8);
    const setupChain=(hot,deviceLoad=false)=>{reset(baseCase);cpu.segment_offsets.fill(0,0,6);cpu.reg32[6]=BASE+0x40;cpu.reg32[7]=BASE+0x10040;mem.fill(0xCC,BASE+0x10040,BASE+0x10050);mem.set(chainBytes,PC);e.full_clear_tlb();if(hot){e.ir_memory_write(BASE+0x40,mem[BASE+0x40],1);e.ir_memory_write(BASE+0x10040,mem[BASE+0x10040],1);}if(deviceLoad){set32(0x13000+0x310*4,0xA0007);e.full_clear_tlb();onEvent=kind=>{if(kind.startsWith("r")){xmm[4]=0xDEADBEEF;xmm[24]=0xDEADBEEF;}};}slow=guards=0;events=[];};
    let chains=0;
