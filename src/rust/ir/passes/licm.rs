...
// size before verification or cloning, without following potentially invalid IDs.
...
/// Arena and aggregate metadata caps bound verification/cloning inputs; those
/// operations are separate from the discovery/candidate work counter.
pub fn run(region: &mut Region, config: Config) -> Result<Stats, String> {
    ...
    // No legal loop means no mutation is possible. Avoid a full HIR clone.
    if loops.is_empty() {
        return Ok(Stats {
    ...
    // With no motion, the input was already verified and its arenas need not
    // be replaced. Every transformed candidate still passes final validation.
    if stats.hoisted != 0 {
...
fn natural_loops(region: &Region, cfg: &Cfg, budget: &mut Budget) -> Result<Vec<NaturalLoop>, String> {
