...
| IR-13 | 完整矩阵未完成 | 已有 host/browser/device acceptance gate，并完成第一轮同核 execution-budget 矩阵：128→1024 时 synthetic hot-loop 吞吐由约 364k 提升到 691k steps/ms（约 1.90x），但仍约为 paired legacy 的 55%；已修正“guest steps / dispatcher budget”单位不一致的 utilization 指标，下一轮改用 exact target-entry counters 并扩展 2048/4096 估计 activation overhead 渐近上限。XP、真实应用/游戏、稳定冷热性能阈值及剩余 IR-02～IR-09 回补仍未完成，详见 ir13-acceptance.md 与 ir13-tuning.md |
