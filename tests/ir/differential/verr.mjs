...
try {
    ...
    for(const [c,i] of selected){
        if(c[5]===8){
            ...
            assert.equal(actual.ip,PF);assert.equal(actual.regs[4],STACK-24);source_faults++;
        }
        for(const initial_zero of [false,true]){
            const actual=compare(i,()=>{reset(i,{value:0x1000,cpl:3});cpu.flags[0]=cpu.flags[0]&~64|(+initial_zero<<6);cpu.reg32[4]=0x12348000;mem.copyWithin(0x5000,0x3000,0x3040);cpu.gdtr_offset[0]=0x5000;cpu.gdtr_size[0]=0x10FF;set32(0x13000+5*4,0x5003);set32(0x13000+6*4,0);e.full_clear_tlb();},101);
            assert.equal(actual.ip,PF);assert.equal(actual.cr[2],0x6000);
            assert.equal(!!(get32(STACK-12)&64),initial_zero);descriptor_faults++;
        }
    }
    for(let i=0;i<cases.length;i++){
    ...
    console.log(`PASS (${release?"release":"debug"}): ${source_faults} source #PF, ${descriptor_faults} descriptor #PF/raw ZF frames, ${segments} segment-priority cases`);
    const observe=(kind,a,value)=>{events.push({kind,a,value,regs:Array.from(cpu.reg32,x=>x>>>0),flags:e.get_eflags()>>>0,rawZero:cpu.flags[0]&64,ip:cpu.instruction_pointer[0]>>>0});on_event?.(kind,a,value);};
    ...
    for(const [c,i] of selected){
        for(const raw_zero of [0,64]){
            compare(i,()=>{reset(i);cpu.flags[0]=cpu.flags[0]&~64|raw_zero;if(c[5]===8)set32(0x13000+0x310*4,0xA0003);set32(0x13000+3*4,0xA1003);e.full_clear_tlb();});
            const descriptor=events.filter(x=>x.a>=0xA1000);assert(descriptor.length>0);
            assert(descriptor.every(x=>(x.flags&64)===raw_zero));devices++;
        }
